...
class GPUPolicyClient:
    ...
    def _process_image(self, compressed_bytes, original_shape):
        """Process compressed image bytes into normalized tensor"""
        try:
            # Decode JPEG image
            img_array = np.frombuffer(compressed_bytes, dtype=np.uint8)
            ...
            print(f"[CLIENT] Error processing image: {e}", flush=True)
            # Return blank image if processing fails
            return torch.zeros(1, 3, *original_shape[:2]).to(self.device)
    
    def _compute_action(self, observation_frame):
        """Compute action using the policy model"""
        try:
            # Process images
            kinect_rgb = self._process_image(
                observation_frame["observation.image.kinect_rgb"],
                observation_frame["observation.image.kinect_rgb_shape"]
            )
            
            c922_webcam = self._process_image(
                observation_frame["observation.image.c922_webcam"],
                observation_frame["observation.image.c922_webcam_shape"]
            )
            
            # Process state (already a tensor)
            state = observation_frame["observation.state"].to(self.device)
            
            # Create observation dictionary
            processed_obs = {
                "observation.image.kinect_rgb": kinect_rgb,
                "observation.image.c922_webcam": c922_webcam,
                "observation.state": state,
                "task": ["push the block forward"]
            }
            ...
            print(f"[CLIENT] Computed action: {action}", flush=True)
            return action
    ...
    def _send_action(self, action):
        ...
        try:
            if action is None:
                print("[CLIENT] No action to send", flush=True)
                return
...
def main():
    ...
    POLICY_PATH = "outputs/train/smol_UR10_29jul_250ep/checkpoints/last/pretrained_model/" 
