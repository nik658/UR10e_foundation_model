...
class CameraManager:
    def __init__(self, fps=15):
        self.fps = fps
        self.rgb_frame = None
        self.c922_frame = None  # Changed from depth_frame
        self.running = False
        self.initialized = False
        self.frame_lock = threading.Lock()
        
        # Initialize C922 webcam
        self.c922_cap = None
    ...
    def stop(self):
        ...
        # Release C922 webcam
        if self.c922_cap is not None:
            self.c922_cap.release()
            self.c922_cap = None
    ...
    def _camera_loop(self):
        ...
        # Initialize C922 webcam
        self.c922_cap = cv2.VideoCapture("/dev/video2")
        self.c922_cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        self.c922_cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        
        if not self.c922_cap.isOpened():
            print("[CAMERA] Cannot open C922 webcam", flush=True)
            self.c922_cap = None
        else:
            print("[CAMERA] C922 webcam initialized successfully", flush=True)
        ...
        while self.running:
            try:
                ...
                if rgb is None:
                    print("[CAMERA] No valid RGB frames")
                    rgb = np.zeros((480, 640, 3), dtype=np.uint8)
                
                rgb_resized = cv2.resize(rgb, (640, 480))
                
                # Get C922 frame
                c922_frame = np.zeros((640, 480, 3), dtype=np.uint8)  # Default fallback
                if self.c922_cap is not None:
                    ret, frame = self.c922_cap.read()
                    if ret:
                        c922_frame = cv2.resize(frame, (640, 480))
                        # Convert BGR to RGB for consistency
                        c922_frame = cv2.cvtColor(c922_frame, cv2.COLOR_BGR2RGB)
                    else:
                        print("[CAMERA] Failed to read from C922 webcam")
                
                with self.frame_lock:
                    self.rgb_frame = rgb_resized.copy()
                    self.c922_frame = c922_frame.copy()
                ...
                with self.frame_lock:
                    ...
                    self.c922_frame = np.zeros((480, 640, 3), dtype=np.uint8)
                    if not self.initialized:
                        self.initialized = True
    ...
    def get_latest_frames(self):
        ...
        with self.frame_lock:
            if self.rgb_frame is not None and self.c922_frame is not None:
                return self.rgb_frame.copy(), self.c922_frame.copy()
            else:
                print("[CAMERA] NO valid frames in get_latest_frames")
...
class RobotPolicyServer:
    def __init__(self, host='0.0.0.0', port=5000, fps=15):
        ...
        # Define observation features - Updated for C922
        self.obs_features = {
            ...
            "observation.image.c922_webcam": {  # Changed from kinect_depth
                "dtype": "video",
                "shape": (480, 640, 3),  # RGB 3 channels
                "names": ["height", "width", "channel"]
            },
    ...
    def _initialize_robot(self):
        ...
        try:
            ...
            if robot_connected:
                ...
                print("[SERVER] robot home state", self.home_pose)
                print("[SERVER] moving robot to check", self.robot.send_tcp_action([0.01,0.01,0.01]))
            else:
    ...
    def _get_observation(self):
        ...
        rgb, c922 = self.camera_manager.get_latest_frames()  # Changed variable name
        
        # Ensure RGB and C922 are numpy arrays
        ...
        rgb_resized = cv2.resize(rgb, (640, 480))
        c922_resized = cv2.resize(c922, (640, 480))

        obs = {
            "observation.image.kinect_rgb": rgb_resized,
            "observation.image.c922_webcam": c922_resized,  # Changed key name
    ...
    def _build_observation_frame(self, observations):
        ...
        if "observation.image.kinect_rgb" in observations:
            img = observations["observation.image.kinect_rgb"]
            if img.dtype != np.uint8:
                img = (img * 255).astype(np.uint8)
            
            _, compressed_img = cv2.imencode('.jpg', img, [cv2.IMWRITE_JPEG_QUALITY, 80])
            compressed_bytes = compressed_img.tobytes()
            return_observations["observation.image.kinect_rgb"] = compressed_bytes
            return_observations["observation.image.kinect_rgb_shape"] = img.shape

        # Handle C922 webcam observation - Updated section
        if "observation.image.c922_webcam" in observations:
            c922_img = observations["observation.image.c922_webcam"]
            
            if c922_img.dtype != np.uint8:
                c922_img = (c922_img * 255).astype(np.uint8)
            
            _, compressed_c922 = cv2.imencode('.jpg', c922_img, [cv2.IMWRITE_JPEG_QUALITY, 80])
            compressed_c922_bytes = compressed_c922.tobytes()
            
            return_observations["observation.image.c922_webcam"] = compressed_c922_bytes
            return_observations["observation.image.c922_webcam_shape"] = c922_img.shape
    ...
    def _execute_action(self, action):
        ...
        try:
            ...
            if action is None:
                print("[SERVER] None action- not executing")
                return 
    ...
    def _handle_client_connection(self):
        ...
        try:
            while self.running:
                ...
                # Get observation
                print("[SERVER] getting obs")
                obs = self._get_observation()

                try:
                    observation_frame = self._build_observation_frame(obs)
                except Exception as e:
                    print(f"[SERVER] Error during frame building: {e}", flush=True)
                    raise
